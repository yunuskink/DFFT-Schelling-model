...
function update_counts_single_dynamics!(lattice,bins,bin_IDs,counts_single,bin_counts_dt,t_row)
    #num_blue = Array{UInt16,1}(UndefInitializer(), length(bins))
    #num_red = similar(num_blue)
    for ID in bin_IDs
        num_blue_block = sum(lattice[bins.==ID].==1);
        # num_red_block = sum(lattice[bins.==ID].==2);
        counts_single[num_blue_block+1] += 1;
        bin_counts_dt[t_row,ID] = num_blue_block
    end
    #num_blue_block = [sum(lattice[bins.==ID].==1) for ID in bin_IDs];
    #num_red_block = [sum(lattice[bins.==ID].==2) for ID in bin_IDs];
    return counts_single
...
function run_schelling_sim_binary(;lattice_length = 60,
    ...
    if collect_dynamics
        bin_counts_dt = zeros(Int(num_simulation_steps/ dt),length(bin_IDs))
        t_row = 0
    end
    ...
    for step = 1:num_simulation_steps
        ...
        if collect_dynamics
            if mod(step,dt)==0
                t_row += 1
                update_counts_single_dynamics!(lattice, bins, bin_IDs, counts_single, bin_counts_dt, t_row)
            else
    ...
    if collect_dynamics
        return counts_single, lattice_snapshots,utility_function_blue,utility_function_red, bin_counts_dt
    else
