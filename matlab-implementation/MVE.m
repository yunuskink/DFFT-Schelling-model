...
% OUTPUT
% Mean1(i,j) gives the (unscaled) mean number of "1" agents at time i in
% block j as predicted by the unscaled MVE
% Mean2(i,j) gives the (unscaled) mean number of "2" agents at time i in
% block j as predicted by the unscaled MVE
...
f=gf;
f=RemovePlane(f);
Edgef=1; % Not a necessary variable for the MVE equation as stated in the main text
[D1,D11,D2,D22]=Derivative3(f,s,Edgef);
...
Mean1=zeros(T,Nblock);
Mean2=Mean1;

% Find Initial N1 and N2
N1=mean(Data1(1,:,:),3);%% Overview

%This script demonstrates how to run Schelling simulations and collect
%observations of block densities and then perform DFFT analysis and
%predictions on those densities.
...
for i=2:T
    disp(i)
    for k=1:step
        [N1,N2]=EvolveDensity(N1,N2,N,E,Nblock,s,V1,V2,D1,D11,D2,D22);
    end
    Mean1(i,:)=N1';
    Mean2(i,:)=N2';
end
...
plot(Mean1);
xlabel("Time")
ylabel("Average # Blue agents")

figure;
plot(Mean2);
xlabel("Time")
ylabel("Average # Red agents")


%% Functions 
function [D1,D11,D2,D22]=Derivative3(f,s,Edgef)
    % Take partial derivatives (scaled) of f along first and second dimension
    % The edges of f is set to be Edgef to restrict the trajectories of
    % the MVE to possible region. This is not necessary for the MVE alone,
    % but might help if we want to introduce a langevin-like noise term.
    F=NaN(s+3,s+3);
    F(:,1)=Edgef;
    F(1,:)=Edgef;
    F(2:end-1,2:end-1)=f;
    ...
    F=f(:,2:end-1);
    ad=[F(2:end,:);nan(1,size(F,2))];
    subt=[nan(1,size(F,2));F(1:end-1,:)];
    D11=ad-F;
    D1=F-subt;
    D1=D1(2:end-1,:);
    D11=D11(2:end-1,:);
    ...
    F=f(2:end-1,:);
    ad=[F(:,2:end),nan(size(F,1),1)];
    subt=[nan(size(F,1),1),F(:,1:end-1)];
    D22=ad-F;
    D2=F-subt;
    D2=D2(:,2:end-1);
    D22=D22(:,2:end-1);
...
function [N1,N2]=EvolveDensity(N1,N2,N,E,Nblock,s,V1,V2,D1,D11,D2,D22)
% Evolves density for one step for red and blue densities
    N1i=repmat(N1,1,Nblock);
    N2i=repmat(N2,1,Nblock);
    N1f=repmat(reshape(N1,1,Nblock),Nblock,1);
    N2f=repmat(reshape(N2,1,Nblock),Nblock,1);
    V1i=repmat(V1,1,Nblock);
    V2i=repmat(V2,1,Nblock);
    V1f=repmat(reshape(V1,1,Nblock),Nblock,1);
    V2f=repmat(reshape(V2,1,Nblock),Nblock,1);
    
    
    [x,y]=meshgrid(0:s);
    D1i=interp2(x,y,D1,N2i,N1i);
    D1f=interp2(x,y,D11,N2f,N1f);
    D2i=interp2(x,y,D2,N2i,N1i);
    D2f=interp2(x,y,D22,N2f,N1f);
    
    % Red
    DeltaH=(V1f+D1f)-(V1i+D1i);
    Rate1=(N1i/N).*((s-N1f-N2f)/E).*(1./(exp(DeltaH)+1));
    
    % Blue
    DeltaH=(V2f+D2f)-(V2i+D2i);
    Rate2=(N2i/N).*((s-N1f-N2f)/E).*(1./(exp(DeltaH)+1));
